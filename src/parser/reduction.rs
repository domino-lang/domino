...
use crate::packageinstance::{game_inst_type_mapping, game_inst_type_mapping_vec};
...
use super::{
    ...
    theorem::{ParseTheoremContext, ParseTheoremError},
    Rule,
};
...
pub(super) fn handle_reduction_body<'src>(
    ...
    let (mapping1, parsed_name1) = handle_mapspec_assumption(ctx, map1_ast, assumption, is_hybrid)?;
    let (mapping2, parsed_name2) = handle_mapspec_assumption(ctx, map2_ast, assumption, is_hybrid)?;

    let (mapping_left, mapping_right) = if parsed_name1 != right_name {
        // we know mapping1 should be for left and mapping2 should be for right

        if parsed_name1 != left_name {
...
fn handle_mapspec_assumption<'src>(
    ...
    let assumption_game_inst_name_ast: GameInstanceName = ast.next().unwrap().into();
    let construction_game_inst_name_ast = ast.next().unwrap();
    // let (
    ...
    let construction_game_inst_name = if is_hybrid {
        ...
        construction_game_inst_name_ast.as_str()
    };
    ...
    let mapping = ReductionMapping {
        ...
        construction: construction_game_inst_name_ast.into(),
        entries: mappings
            .into_iter()
    ...
    Ok((mapping, construction_game_inst_name.to_string()))
}
...
fn package_instances_diff(
    ...
    for (pkg_ty, left_game_ty) in &left_pkg_inst.types {
        ...
        let left_rules: Vec<_> = game_inst_type_mapping(&left_game_inst.types)
            .chain(left_game_inst.consts.iter().filter_map(|(ident, _expr)| {
                if ident.ty.kind().is_integer() {
                    ...
                    // TODO: do we need to also do something for integer literals here?
                    ident
                        .assigned_value
                        .clone()
                        .unwrap()
                        .as_identifier() // Could be a literal or an identifier
                        .map(|assigned| {
                            (
                                Type::bits(CountSpec::Identifier(Box::new(named.into()))),
                                Type::bits(CountSpec::Identifier(Box::new(assigned.clone()))),
                            )
                        })
                } else {
                    None
            ...
            .collect();
        let right_rules: Vec<_> = game_inst_type_mapping(&right_game_inst.types)
            .chain(right_game_inst.consts.iter().filter_map(|(ident, _expr)| {
                if ident.ty.kind().is_integer() {
                    let mut named = ident.clone();
                    named.game_inst_name = None;
                    named.theorem_name = None;
