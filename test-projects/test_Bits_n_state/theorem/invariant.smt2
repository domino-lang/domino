...
(define-fun invariant
  ...
  (let
    ...
    ; getting test-constant out of state
      (test_constant-0 (<pkg-state-Rand-<$<!n!>$>-test_constant> (<game-SmallComposition-<$<!n!>$>-pkgstate-rand> state-0)))
      (test_constant-1 (<pkg-state-Fwd-<$<!n!>$>-test_constant> (<game-MediumComposition-<$<!n!>$>-pkgstate-fwd> state-1)))
    )
    (and
    ; ctr are equal
    (= ctr-0 ctr-1)
    ; test-constants are equal
    (= test_constant-0 test_constant-1)
    )
 ))
