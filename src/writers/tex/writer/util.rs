...
pub(super) fn list_to_matrix<S: Borrow<str>>(mut it: impl Iterator<Item = S>) -> String {
    ...
    loop {
        ...
        match it.next() {
            ...
            Some(s) => {
                if len + s.borrow().len() > 30 {
                    line.push(String::new());
                    lines.push(line.join(", "));
                    line = Vec::new();
                    len = 0;
                }
                line.push(s.borrow().to_string());
                len = len + std::cmp::max(6, s.borrow().len()) - 4 // latex makes string length and text length quite different
            }
        }
    }
    format!(
