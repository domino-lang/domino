...
//! Building `games/<Comp>.ec` (`docs/stories/easycrypt/04-games-and-router.md`
//! §3.2): one file per distinct **composition** — several game instances can
//! share one composition (`theorem.instances` in `Simple4WHS` maps `Real`,
//! `Ideal` and `Hybrid0` all onto the `Hybrid0` composition), so this emits
//! per composition, not per instance, deduplicated by
//! [`interfaces::discover_compositions`].
//!
//! Each file gets: one `clone <Variant> as Pkg_<inst>.` per package instance
//! (story 03's variants, resolved here via the same [`package::VariantKey`]
//! machinery story 03 uses internally); a `module Inst_<inst> = ...`
//! functor-application alias for every instance that imports oracles; a
//! router module (`Game_<Comp>`) carrying the single `abort_flag : bool` and
//! one export proc per `comp.exports` entry; and an experiment module
//! (`Exp_<Comp>`) that initialises the router and runs the adversary.
...
fn render_game_file(
    ...
    plain_requires.extend(variant_requires);
    ...
    Ok(EcFile {
        header: vec![format!(
            "games/{mangled}.ec for theorem `{theorem_name}`, composition `{}` — generated by \
             `domino easycrypt`.",
            comp.name
        )],
        requires: vec![
...
mod tests {
    ...
    fn hello_world_games_match_golden() {
        ...
        for f in &files {
            assert_golden(
                f,
                &format!("testdata/easycrypt/story04/hello-world/games/{}.ec", f.name),
            );
        }
    }
    ...
    fn simple_4whs_games_match_golden() {
        ...
        for f in &files {
            assert_golden(
                f,
                &format!("testdata/easycrypt/story04/4WHS/games/{}.ec", f.name),
            );
        }
    }
    ...
    fn simple_4whs_games_compile() {
        for name in ["Hybrid0", "Hybrid1", "Hybrid2", "PRF_Game"] {
            assert_compiles(
