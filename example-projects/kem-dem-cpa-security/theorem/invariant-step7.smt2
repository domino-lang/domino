...
(define-state-relation relation-randomness
    ...
    (and 
        (rand-is-eq 
            ...
            (sample-id "pkg_DemScheme" "DEM_ENC" "dem_enc")
            (get-rand-ctr-hybrid2_1 (sample-id "pkg_DemScheme" "DEM_ENC" "dem_enc"))
            (get-rand-ctr-game_1 (sample-id "pkg_DemScheme" "DEM_ENC" "dem_enc"))
        )
        (rand-is-eq 
            (sample-id "pkg_KemScheme" "KEM_ENCAPS" "kem_encaps")
            (sample-id "pkg_KemScheme" "KEM_ENCAPS" "kem_encaps")
            (get-rand-ctr-hybrid2_1 (sample-id "pkg_KemScheme" "KEM_ENCAPS" "kem_encaps"))
            (get-rand-ctr-game_1 (sample-id "pkg_KemScheme" "KEM_ENCAPS" "kem_encaps"))
        )
        (rand-is-eq 
            (sample-id "pkg_KEM_CPA" "ENCAPS" "k")
            (sample-id "pkg_Game" "PKENC" "k")
            (get-rand-ctr-hybrid2_1 (sample-id "pkg_KEM_CPA" "ENCAPS" "k"))
            (get-rand-ctr-game_1 (sample-id "pkg_Game" "PKENC" "k"))
        )
    )
)
