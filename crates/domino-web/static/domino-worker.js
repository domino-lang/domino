import init, { load, proofsteps, prove, projectdetails,
               list_packages, list_games, list_theorems,
               get_package, get_game, get_theorem } from './domino/domino_web.js';
...
onmessage = (e) => {
    console.log("worker", e.data.func, e.data);
    switch(e.data.func) {
    case "load": {
    ...
    case "get-package-code": {
        let files = files_collection.get(e.data.project);
        let result = get_package(files, e.data.packagename);
        postMessage({
            func: "get-package-code",
            data: result,
            packagename: e.data.packagename,
            project: e.data.project,
        });
        break
    }
